...
class CNN(NeuralNetwork):
    ...
    def create_model(self, dataset='mnist'):
        ...
        if dataset.lower() == 'mnist' or dataset.lower() == 'mnist-big':
            layers = [
                Conv2D(6, (5, 5), padding='valid', input_shape=(self.input_side, self.input_side, self.input_channels), name='conv1'),
                Activation(self.non_linearity),
                Conv2D(16, (5, 5), name='conv2'),
                Activation(self.non_linearity),
                Dropout(self.dropout_prob),
                Flatten(),
                Dense(120, name='dense1'),
                Activation(self.non_linearity),
                Dropout(self.dropout_prob),
                Dense(84, name='dense2'),
                Activation(self.non_linearity),
                Dense(self.num_classes, name='logits'),
                Activation('softmax')
            ]
            
        elif dataset.lower() == 'fashion_mnist' or dataset.lower() == 'fashion_mnist-big':
            layers = [
                Conv2D(6, (5, 5), padding='valid', input_shape=(self.input_side, self.input_side, self.input_channels), name='conv1'),
                Activation(self.non_linearity),
                Conv2D(16, (5, 5), name='conv2'),
